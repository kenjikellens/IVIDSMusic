...
[2026-03-07] EDITED app/src/main/assets/gui/index.css: Added widescreen breakpoint at min-width 2000px with --ui-base-scale: 1.1 for large monitors and TVs.
[2026-03-07] EDITED app/src/main/assets/gui/index.css: Added 4K breakpoint (min-width: 3000px, scale 1.25) and 5K/8K breakpoint (min-width: 5000px, scale 1.5).
[2026-03-07] EDITED app/src/main/assets/gui/index.css: Visual mobile portrait polish — added safe-area inset support for notch/gesture bars, refined player bar visual hierarchy (larger art, tighter text spacing), cleaned up hero blobs (hidden in portrait for performance), aligned horizontal card row edge padding, refined spacing and border-radiuses, and disabled all hover scale effects (`transform: scale(1)!important;`) for interactive elements in portrait mode to eliminate tap jitter on touch screens.
